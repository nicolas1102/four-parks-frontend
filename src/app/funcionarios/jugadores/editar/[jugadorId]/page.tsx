...
const Page = ({ params }: PageProps) => {
  const { jugadorId } = params
  const router = useRouter()
  const { getOneJugador, updateJugador } = useJugador()
  ...
  useEffect(() => {
    const loadJugador = async () => {
      const jugador = await getOneJugador(jugadorId)
      if (jugador) {
        setValue('nombre', jugador.nombre)
        setValue('apellido', jugador.apellido)
        setValue('nacionalidad', jugador.nacionalidad)
        setValue('minutos_jugados', jugador.minutos_jugados)
        setValue('edad', jugador.edad)
  ...
  }, [getOneJugador, jugadorId, setValue])
      ...
      {isLoading ? (
        ...
        <div className='mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[400px] border p-3 mt-2'>
          <div className='flex flex-col items-center space-y-1 text-center'>
            ...
            <h1 className='text-2xl tracking-widest p-3'>EDITAR JUGADOR</h1>
                      ...
                      if (equipo._id) {
                        return (
                          <option
                            ...
                            value={equipo.nombre}
                            selected={
                              getValues('equipo') === equipo.nombre
                                ? true
                                : false
                            }
                          >
